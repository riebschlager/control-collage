...
void ofApp::render() {
    ...
    if(ofRandom(1) < jitterScaleFrequency) scale *= ofRandom(jitterScaleAmount);
    float rotation = ofMap(noise, 0, 1, minRotation, maxRotation);
    int sliceIndex = floor(ofMap(noise, 0, 1, minSliceIndex, maxSliceIndex));
    if(ofRandom(1) < randomSliceFrequency) sliceIndex = floor(ofRandom(slices.size()));
    ...
    ofSetColor(alpha);
    if(noise > 0.5) slices[sliceIndex].draw(0, 0);
    ofPopMatrix();
    canvas.end();
}
...
void ofApp::update(){
    time += timeScale;
    if(ofRandom(1) < timeSkipFrequency) time += timeSkipAmount;
